...
1. Проверить health. Если `502/503` от Cloudflare — смотри **Dashboard → Deployments**: не упал ли последний деплой
2. Открыть **Deployments** — последний production-деплой должен быть зеленым. Если нет, то последний деплой сломал прод:
   ```
   Dashboard → webapp → Deployments → выбери предыдущий рабочий → Rollback to this deployment
   ```
   (или передеплоить предыдущий хороший коммит)
...
2. Проверить секрет: **Dashboard → webapp → Settings → Environment variables** — есть ли `PAYPAL_CLIENT_SECRET` (если нет → `getPayPalAccessToken` вернёт null → 503)
3. Проверить, что `PAYPAL_CLIENT_ID` (wrangler.jsonc) и секрет — из одного приложения PayPal, и `PAYPAL_MODE` совпадает (sandbox)
...
2. Проверить, применены ли миграции:
   ```bash
...
- **Метрики**: Dashboard → webapp → Metrics 
- **Алерты**: Dashboard → Notifications → Add → "Pages deployment failed" + (через Health Checks / Workers) на доступность
- **Бизнес-метрики**: админ-панель `/admin` → секция «Бизнес-метрики» (конверсия/выручка/ARPU) + PostHog 
